...
class KMeans:
    ...
    def run(self, points, random_seed):
        ...
        for _ in range(self._num_iterations):
            ...
            for point in points:
                distances_to_clusters = {x.id: point.distance_to(
                    x.centroid) for x in self._clusters}
                closest_cluster_id = sorted(
                    distances_to_clusters.keys(), key=lambda x: distances_to_clusters[x])[0]
    ...
    def print_results(self, random_seed):
        ...
        # print('############################################')
        sl1 = total_loss
        print(f"{random_seed} {self._k} {sl1}")
