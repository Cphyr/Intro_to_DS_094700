...
class Cluster:
    ...
    def remove_point(self, point=None):
        ...
        if not point:
            ...
            print('Point with name {} does not belong to cluster {}'.format(
                point.name, self.id))
        else:
    ...
    def compute_loss(self):
        distances = [self._centroid.distance_to(
            point.coordinates) for point in self._points]
        return sum(distances)

    def compute_SSE(self):
        errors = [self._centroid.distance_to(
            point.coordinates)**2 for point in self._points]
        return sum(errors)
